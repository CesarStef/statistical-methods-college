...
We can also see that the variables `Outstate` and `Room-and-board` are directly highly correlated, which is easily explainable by the fact that the more out-of-state students there are, the more expensive room and board will become.

`Private` and `F.Undergrad` are inversaly midly correlated and more then `Private` and `Apps` which is can be explainable by the fact that private colleges have tipical more graduated students than undergraduated.

On the other side, `Private` and `Outstate` which are directly midly correlated. This can be explained by the fact that private colleges are more expensive than public ones and so, if some student can afford to for out-of-state tuition they can also afford to pay a private college.
...
??? CHAT GPT WHY WE SHOULD DO THAT??? Now we can see the correlation matrix of the cleared dataset

```{r}
correlation_matrix_cleared <- cor(college_cleared)
corrplot(correlation_matrix_cleared, method="color", tl.cex = 0.8)
```

Now we can try a glm with all the usable parameters:
...
```{r}
glm_all_parameters_train_data <- glm(College$Apps[train_index] ~ .,data=train_data, family=poisson)

glm_all_parameters <- glm(College$Apps ~ .,data=college_cleared, family=poisson)

summary(glm_all_parameters)
summary(glm_all_parameters_train_data)
```
...
rmse_train_set <- RMSE(predict.glm(glm_all_parameters_train_data, newdata = test_data), College$Apps[-train_index])
rmse_test_set <- RMSE(predict.glm(glm_all_parameters_train_data), College$Apps[train_index])
```
