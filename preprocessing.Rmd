...
# We can also see that the variables `Top10perc` and `Top25perc` are directly heavly correlated. For the same reason we can exclude `Top10perc` in our case.
...
# We can also see that the variables `Private` and `F.Undergrad` are inversaly midly correlated and more then `Private` and `Apps` which is can be explainable by the fact that private colleges have tipical more graduated students than undergraduated.

# The opposite situation is with `Private` and `Outstate` which are directly midly correlated. This can be explained by the fact that private colleges are more expensive than public ones and so, if some student can afford to for out-of-state tuition they can also afford to pay a private college.
